...
# def sigmoid(x):
#     y = 1/(1+np.exp(-x/x_scale))
#     return y.round(decimals=2)


def relu_LUT_gen(num_entries = 255, step = 5):
    LUT_list = list(range(0, (num_entries - 1) * step, step))
    LUT_list = [-99999999] + LUT_list
    return LUT_list
...
def run(AF='relu', relu_scale=1, plot=True):

    print("********************run AF={}, relu scale={} start********************".format(AF, relu_scale))
    if AF == "sigmoid":
        weight_scale = 30
        x_scale = 5
        ...
        x_scale = 1
        sum_scale = weight_scale * x_scale
        look_up_table = relu_LUT_gen(num_entries = 255, step = relu_scale)
        # print(look_up_table)
    ...
    # nthimage = 7
    # print(mnist.test.images[nthimage])
    # image = mnist.test.images[nthimage].reshape(28, 28) * 255
    # im = Image.fromarray(image.reshape(28, 28))
    # im.show()
    # print("True:       ", np.argmax(mnist.test.labels[nthimage]))
    #
    #
    # with tf.Session() as sess:
    #     x1 = tf.sigmoid(np.matmul(mnist.test.images[nthimage], w1) + b1).eval()
    #     x2 = tf.sigmoid(np.matmul(x1, w2) + b2).eval()
    #     x3 = tf.sigmoid(np.matmul(x2, w3) + b3).eval()
    #     x4 = np.matmul(x3, w4) + b4
    #     print("Predict:    ", np.argmax(x4))

    w1 = (w1 * weight_scale).astype(int)
    w2 = (w2 * weight_scale).astype(int)
    w3 = (w3 * weight_scale).astype(int)
    w4 = (w4 * weight_scale).astype(int)
    b1 = (b1 * sum_scale).astype(int)
    b2 = (b2 * sum_scale).astype(int)
    b3 = (b3 * sum_scale).astype(int)
    b4 = (b4 * sum_scale).astype(int)

    # [w1, w2, w3, w4] = map(threshold, [w1, w2, w3, w4])

    correct = 0
    t = time.time()
    for nthimage in range(num_test_samples):
        label = np.argmax(y_test[nthimage])
        normedimage = X_test[nthimage]
        normedimage = normedimage.round(decimals=2)

        y1 = np.add(np.matmul(normedimage*x_scale, w1), b1)
        x1 = LUTquery(y1, look_up_table)
        if AF == 'relu': x1 *= relu_scale
        y2 = np.add(np.matmul(x1*x_scale, w2), b2)
        x2 = LUTquery(y2, look_up_table)
        if AF == 'relu': x2 *= relu_scale
        y3 = np.add(np.matmul(x2*x_scale, w3), b3)
        x3 = LUTquery(y3, look_up_table)
        if AF == 'relu': x3 *= relu_scale
        y4 = np.add(np.matmul(x3*x_scale, w4), b4)
        predict = np.argmax(y4)

        if label == predict:
            correct += 1
    
    accuracy = float(correct)/num_test_samples
